...
public:
  ...
  static inline void setPAG(PAG *g) { pag = g; }
  //@}
  ...
  inline NodeSet &getAllValidPtrs() { return pag->getAllValidPtrs(); }
...
public:
  ...
  virtual void resolveIndCalls(llvm::CallSite cs, const PointsTo &target,
  ...
  inline bool inSameCallGraphSCC(const llvm::Function *fun1,
  ...
  void dumpAllTypes();
  //@}
  // [OS-CFI] return llvm::value from corresponding node
  virtual const llvm::Value *getValueFromNodeID(NodeID);

  /// get CHGraph
  CHGraph *getCHGraph() const { return chgraph; }
  ...
  void connectVCallToVFns(llvm::CallSite cs, const VFunSet &vfns,
                          CallEdgeMap &newEdges,
                          llvm::CallGraph *callgraph = NULL);
  virtual void resolveCPPIndCalls(llvm::CallSite cs, const PointsTo &target,
                                  CallEdgeMap &newEdges,
                                  llvm::CallGraph *callgraph = NULL);
...
public:
  ...
  typedef DiffPTData<NodeID, PointsTo, EdgeID>
      DiffPTDataTy; /// Points-to data structure type
  typedef DFPTData<NodeID, PointsTo>
      DFPTDataTy; /// Points-to data structure type
  typedef IncDFPTData<NodeID, PointsTo>
      IncDFPTDataTy; /// Points-to data structure type
  ...
  ///@{
  virtual inline PointsTo &getPts(NodeID id) { return ptD->getPts(id); }
...
public:
  ...
  typedef std::map<NodeID, PointsTo>
      PtrToBVPtsMap; /// map a pointer to its BitVector points-to representation
  typedef std::map<NodeID, CPtSet>
      PtrToCPtsMap; /// map a pointer to its conditional points-to set

  /// Constructor
  CondPTAImpl(PointerAnalysis::PTATY type)
      : PointerAnalysis(type), normalized(false) {
  ...
  /// Get points-to and reverse points-to
  ///@{
  virtual inline CPtSet &getPts(CVar id) { return ptD->getPts(id); }
...
protected:
  ...
  virtual void NormalizePointsTo() {
    ...
    for (typename PTDataTy::PtsMap::const_iterator it = ptsMap.begin(),
                                                   eit = ptsMap.end();
         it != eit; ++it) {
...
public:
  ...
  /// Interface expose to users of our pointer analysis, given Value infos
  virtual inline llvm::AliasResult alias(const llvm::Value *V1,
                                         const llvm::Value *V2) {
  ...
  /// points-to sets
  virtual inline llvm::AliasResult alias(const CPtSet &pts1,
                                         const CPtSet &pts2) {
    CPtSet cpts1;
    ...
    for (NodeSet::iterator nIter = this->getAllValidPtrs().begin();
      ...
      if (this->getPAG()->isValidTopLevelPtr(node)) {
        if (llvm::isa<DummyObjPN>(node)) {
          ...
          llvm::outs() << "Source Loc: "
                       << analysisUtil::getSourceLoc(node->getValue());
        }
