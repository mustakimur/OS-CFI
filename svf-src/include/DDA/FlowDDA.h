...
public:
  ...
  /// Constructor
  FlowDDA(SVFModule m, DDAClient *client)
      : BVDataPTAImpl(PointerAnalysis::FlowS_DDA),
  ...
  bool testIndCallReachability(LocDPItem &dpm, const llvm::Function *callee,
                               CallSiteID csId);
  ...
  /// Finalize analysis
  inline virtual void finalize() { BVDataPTAImpl::finalize(); }
  ...
  /// Override parent method
  virtual inline NodeID getPtrNodeID(const NodeID &var) const { return var; }
  ...
  /// Update call graph.
  //@{
  void updateCallGraphAndSVFG(const LocDPItem &dpm, llvm::CallSite cs,
  ...
  virtual const std::string PTAName() const { return "FlowSensitive DDA"; }
