...
public:
  DDAClient(SVFModule mod)
      : pag(NULL), module(mod), curPtr(0), solveAll(true) {}
  ...
  virtual inline NodeSet &collectCandidateQueries(PAG *p) {
    ...
    if (solveAll)
      ...
      for (NodeSet::iterator it = userInput.begin(), eit = userInput.end();
           it != eit; ++it)
        addCandidate(*it);
    }
  ...
  inline const NodeSet &getCandidateQueries() const { return candidateQueries; }
