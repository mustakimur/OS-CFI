...
static cl::opt<bool> SingleLoad(
    "single-load", cl::init(true),
...
static cl::opt<bool>
    MallocOnly("malloc-only", cl::init(true),
               cl::desc("Only add tainted objects for malloc"));

static cl::opt<bool>
    TaintUninitHeap("uninit-heap", cl::init(true),
                    cl::desc("detect uninitialized heap variables"));

static cl::opt<bool>
    TaintUninitStack("uninit-stack", cl::init(true),
...
void FunptrDDAClient::performStat(PointerAnalysis *pta) {
  ...
  AndersenWaveDiff *ander =
      AndersenWaveDiff::createAndersenWaveDiff(pta->getModule());
  ...
  for (VTablePtrToCallSiteMap::iterator nIter = vtableToCallSiteMap.begin();
    ...
    const PTACallGraph::FunctionSet &callees =
        callgraph->getIndCSCallees(nIter->second);
    totalCallsites++;
    if (callees.size() == 0)
      zeroTargetCallsites++;
