...
static void emitAtomicCmpXchg(CodeGenFunction &CGF, AtomicExpr *E, bool IsWeak,
  ...
  CGF.EmitStoreToMetadata(Old, Val1);
...
static void EmitAtomicOp(CodeGenFunction &CGF, AtomicExpr *E, Address Dest,
  ...
  case AtomicExpr::AO__atomic_load: {
    ...
    CGF.EmitStoreToMetadata(Load, Dest);
    return;
  }
  ...
  case AtomicExpr::AO__atomic_store_n: {
    ...
    llvm::StoreInst *Store = CGF.EmitStoreToMetadata(LoadVal1, Ptr);
  ...
  CGF.EmitStoreToMetadata(Result, Dest);
}
...
RValue AtomicInfo::ConvertIntToValueOrAtomic(llvm::Value *IntVal,
  ...
  CGF.EmitStoreToMetadata(IntVal, CastTemp)
...
void AtomicInfo::EmitAtomicUpdateLibcall(
  ...
  if ((LVal.isBitField() && BFI.Size != ValueSizeInBits) ||
    ...
    CGF.EmitStoreToMetadata(OldVal, DesiredAddr);
  }
...
void AtomicInfo::EmitAtomicUpdateOp(
  ...
  if ((LVal.isBitField() && BFI.Size != ValueSizeInBits) ||
      requiresMemSetZero(getAtomicAddress().getElementType())) {
    CGF.EmitStoreToMetadata(PHI, NewAtomicIntAddr);
  }
...
void CodeGenFunction::EmitAtomicStore(RValue rvalue, LValue dest,
  ...
  if (LVal.isSimple()) {
    ...
    llvm::StoreInst *store = EmitStoreToMetadata(intValue, addr);
