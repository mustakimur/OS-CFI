...
llvm::Value *CodeGenFunction::EmitBlockLiteral(const CGBlockInfo &blockInfo) {
  ...
  auto storeField =
    [&](llvm::Value *value, unsigned index, CharUnits offset,
        const Twine &name) {
      EmitStoreToMetadata(value, projectField(index, offset, name));
    };
  ...
  if (blockDecl->capturesCXXThis()) {
    ...
    EmitStoreToMetadata(LoadCXXThis(), addr);
  }
  ...
  for (const auto &CI : blockDecl->captures()) {
    ...
    if (CI.isByRef()) {
      ...
      EmitStoreToMetadata(byrefPointer, blockField);
    ...
    // If it's a reference variable, copy the reference into the block field.
      ...
      EmitStoreToMetadata(src.getPointer(), blockField);
    ...
    // If type is const-qualified, copy the value into the block field.
      ...
      EmitStoreToMetadata(value, blockField);
...
CodeGenFunction::GenerateBlockFunction(GlobalDecl GD,
  ...
  if (CGM.getCodeGenOpts().OptimizationLevel == 0) {
    ...
    EmitStoreToMetadata(BlockPointer, Alloca);
  ...
  for (const auto &CI : blockDecl->captures()) {
    ...
    EmitStoreToMetadata(capture.getConstant(), alloca);
    ...
    // call to its copy constructor.
      ...
      if (CopiedCapture.Kind == BlockCaptureEntityKind::ARCStrong) {
        ...
        if (CGM.getCodeGenOpts().OptimizationLevel == 0) {
          ...
          EmitStoreToMetadata(null, dstField);
...
public:
  ...
  void emitCopy(CodeGenFunction &CGF, Address destField,
    ...
    if (CGF.CGM.getCodeGenOpts().OptimizationLevel == 0) {
      CGF.EmitStoreToMetadata(null, destField);
      CGF.EmitARCStoreStrongCall(destField, value, /*ignored*/ true);
      CGF.EmitARCStoreStrongCall(srcField, null, /*ignored*/ true);
      return;
    }
    CGF.EmitStoreToMetadata(value, destField);
    CGF.EmitStoreToMetadata(null, srcField);
  }
...
public:
  ...
  void emitCopy(CodeGenFunction &CGF, Address destField,
    ...
    CGF.EmitStoreToMetadata(copy, destField);
  }
...
void CodeGenFunction::emitByrefStructureInit(const AutoVarEmission &emission) {
  ...
  auto storeHeaderField = [&](llvm::Value *value, CharUnits fieldSize,
    ...
    EmitStoreToMetadata(value, fieldAddr);
