...
static llvm::Value *EmitBitTestIntrinsic(CodeGenFunction &CGF,
  ...
  if (Ordering != llvm::AtomicOrdering::NotAtomic) {
    ...
    if (NewByte)
      CGF.EmitStoreToMetadata(NewByte, ByteAddr);
  }
...
Value *CodeGenFunction::EmitMSVCBuiltinExpr(MSVCIntrin BuiltinID,
  ...
  case MSVCIntrin::_BitScanReverse: {
    ...
    if (BuiltinID == MSVCIntrin::_BitScanForward) {
      ...
      EmitStoreToMetadata(ZeroCount, IndexAddress, false);
    } else {
      ...
      EmitStoreToMetadata(Index, IndexAddress, false);
    }
...
llvm::Function *CodeGenFunction::generateBuiltinOSLogHelperFunction(
  ...
  Address BufAddr(Builder.CreateLoad(GetAddrOfLocalVar(&Params[0]), "buf"),
                  BufferAlignment);
  EmitStoreToMetadata(Builder.getInt8(Layout.getSummaryByte()),
                      Builder.CreateConstByteGEP(BufAddr, Offset++, "summary"));
  EmitStoreToMetadata(Builder.getInt8(Layout.getNumArgsByte()),
  ...
  for (const auto &Item : Layout.Items) {
    ...
    Addr = Builder.CreateBitCast(Addr, Arg.getPointer()->getType(),
                                 "argDataCast");
    EmitStoreToMetadata(Builder.CreateLoad(Arg), Addr);
    Offset += Size;
    ++I;
  }
...
EmitCheckedMixedSignMultiply(CodeGenFunction &CGF, const clang::Expr *Op1,
  ...
  CGF.EmitStoreToMetadata(CGF.EmitToMemory(Result, ResultQTy), ResultPtr,
                          isVolatile);
...
RValue CodeGenFunction::EmitBuiltinExpr(const FunctionDecl *FD,
  ...
  case Builtin::BI__builtin_setjmp: {
    ...
    EmitStoreToMetadata(FrameAddr, Buf);
    ...
    EmitStoreToMetadata(StackAddr, StackSaveSlot);
  ...
  case Builtin::BI__atomic_clear: {
    ...
    if (isa<llvm::ConstantInt>(Order)) {
      int ord = cast<llvm::ConstantInt>(Order)->getZExtValue();
      StoreInst *Store = EmitStoreToMetadata(NewVal, Ptr, Volatile);
      switch (ord) {
      case 0:  // memory_order_relaxed
      default: // invalid order
  ...
  case Builtin::BI__builtin_subcll: {
    ...
    llvm::Value *CarryOut = Builder.CreateZExt(Builder.CreateOr(Carry1, Carry2),
                                               X->getType());
    EmitStoreToMetadata(CarryOut, CarryOutPtr);
  ...
  case Builtin::BI__builtin_mul_overflow: {
    ...
    bool isVolatile =
      ResultArg->getType()->getPointeeType().isVolatileQualified();
    EmitStoreToMetadata(EmitToMemory(Result, ResultQTy), ResultPtr, isVolatile);
  ...
  case Builtin::BI__builtin_smulll_overflow: {
    ...
    EmitStoreToMetadata(Sum, SumOutPtr);
  ...
  case Builtin::BI__builtin_store_halff: {
    ...
    return RValue::get(EmitStoreToMetadata(HalfVal, Address));
  }
  ...
  case Builtin::BI__builtin_ms_va_copy: {
    ...
    return RValue::get(EmitStoreToMetadata(ArgPtr, DestAddr));
  }
  }
...
Value *CodeGenFunction::EmitARMBuiltinExpr(unsigned BuiltinID,
  ...
  case NEON::BI__builtin_neon_vst1_lane_v: {
    ...
    Ty = llvm::PointerType::getUnqual(Ops[1]->getType());
    auto St = EmitStoreToMetadata(Ops[1], Builder.CreateBitCast(PtrOp0, Ty));
    return St;
  }
...
#undef INTRINSIC_X86_XSAVE_ID
  ...
  case X86::BI_umul128: {
    ...
    EmitStoreToMetadata(HigherBits, HighBitsAddress);
  ...
  case X86::BI_InterlockedCompareExchange128: {
    ...
    EmitStoreToMetadata(Builder.CreateExtractValue(CXI, 0), ComparandResult);
...
Value *CodeGenFunction::EmitAMDGPUBuiltinExpr(unsigned BuiltinID,
  ...
  case AMDGPU::BI__builtin_amdgcn_div_scalef: {
    ...
    EmitStoreToMetadata(FlagExt, FlagOutPtr);
    return Result;
  }
...
static Value *EmitSystemZIntrinsicWithCC(CodeGenFunction &CGF,
  ...
  CGF.EmitStoreToMetadata(CC, CCPtr);
...
Value *CodeGenFunction::EmitNVPTXBuiltinExpr(unsigned BuiltinID,
  ...
  case NVPTX::BI__nvvm_match_all_sync_i64p: {
    ...
    EmitStoreToMetadata(Pred, PredOutPtr);
