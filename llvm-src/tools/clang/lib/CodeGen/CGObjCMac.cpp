...
CGObjCMac::GenerateMessageSendSuper(CodeGen::CodeGenFunction &CGF,
  ...
  CGF.EmitStoreToMetadata(Target,
      ...
      if (isa<ObjCAtTryStmt>(S)) {
        if (const ObjCAtFinallyStmt* FinallyStmt =
          ...
          if (CGF.HaveInsertPoint()) {
            CGF.EmitStoreToMetadata(CurCleanupDest,
...
void CGObjCMac::EmitTryOrSynchronizedStmt(CodeGen::CodeGenFunction &CGF,
  ...
  if (!isTry) {
    ...
    CGF.EmitStoreToMetadata(SyncArg, SyncArgSlot);
  }
  ...
  CGF.EmitStoreToMetadata(CGF.Builder.getTrue(), CallTryExitVar);
  ...
  if (!isTry || !cast<ObjCAtTryStmt>(S).getNumCatchStmts()) {
    ...
    CGF.EmitStoreToMetadata(CGF.Builder.getFalse(), CallTryExitVar);
  ...
  // Otherwise, we have to match against the caught exceptions.
    ...
    if (HasFinally) {
      ...
      CGF.EmitStoreToMetadata(Caught, PropagatingExnVar);
    ...
    CGF.EmitStoreToMetadata(CGF.Builder.getInt1(HasFinally), CallTryExitVar);
    ...
    if (HasFinally) {
      ...
      llvm::CallInst *NewCaught =
        CGF.EmitNounwindRuntimeCall(ObjCTypes.getExceptionExtractFn(),
                                    ExceptionData.getPointer(), "caught");
      CGF.EmitStoreToMetadata(NewCaught, PropagatingExnVar);
