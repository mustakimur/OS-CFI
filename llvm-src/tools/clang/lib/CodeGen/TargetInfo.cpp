...
static Address emitVoidPtrDirectVAArg(CodeGenFunction &CGF,
  ...
  llvm::Value *NextPtr =
    CGF.Builder.CreateConstInBoundsByteGEP(Addr.getPointer(), FullDirectSize,
                                           "argp.next");
  CGF.EmitStoreToMetadata(NextPtr, VAListAddr);
...
Address EmitVAArgInstr(CodeGenFunction &CGF, Address VAListAddr, QualType Ty,
  ...
  if (AI.isIndirect()) {
    ...
    CGF.EmitStoreToMetadata(Val, Temp);
    return Temp;
  }
}
...
static Address EmitX86_64VAArgFromMemory(CodeGenFunction &CGF,
  ...
  CGF.EmitStoreToMetadata(overflow_arg_area, overflow_arg_area_p);
...
Address X86_64ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  if (neededInt && neededSSE) {
    ...
    CGF.EmitStoreToMetadata(V, CGF.Builder.CreateStructGEP(Tmp, 1, Offset));
  ...
  if (neededInt) {
    ...
    CGF.EmitStoreToMetadata(CGF.Builder.CreateAdd(gp_offset, Offset),
                            gp_offset_p);
  }
  if (neededSSE) {
    llvm::Value *Offset = llvm::ConstantInt::get(CGF.Int32Ty, neededSSE * 16);
    CGF.EmitStoreToMetadata(CGF.Builder.CreateAdd(fp_offset, Offset),
                            fp_offset_p);
  }
...
Address AArch64ABIInfo::EmitAAPCSVAArg(Address VAListAddr,
  ...
  CGF.EmitStoreToMetadata(NewOffset, reg_offs_p);
  ...
  if (IsHFA && NumMembers > 1) {
    ...
    for (unsigned i = 0; i < NumMembers; ++i) {
      ...
      CGF.EmitStoreToMetadata(Elem, StoreAddr);
    }
  ...
  CGF.EmitStoreToMetadata(NewStack, stack_p);
...
Address SystemZABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  if (IsVector) {
    ...
    llvm::Value *NewOverflowArgArea =
      CGF.Builder.CreateGEP(OverflowArgArea.getPointer(), PaddedSizeV,
                            "overflow_arg_area");
    CGF.EmitStoreToMetadata(NewOverflowArgArea, OverflowArgAreaPtr);
  ...
  CGF.EmitStoreToMetadata(NewRegCount, RegCountPtr);
  CGF.EmitBranch(ContBlock);
...
Address MipsABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  if (DidPromote) {
    ...
    CGF.EmitStoreToMetadata(V, Temp);
    Addr = Temp;
  }
