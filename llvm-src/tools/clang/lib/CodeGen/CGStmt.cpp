...
void CodeGenFunction::EmitReturnOfRValue(RValue RV, QualType Ty) {
  if (RV.isScalar()) {
    EmitStoreToMetadata(RV.getScalarVal(), ReturnValue);
...
void CodeGenFunction::EmitReturnStmt(const ReturnStmt &S) {
  if (requiresReturnValueCheck()) {
    ...
    EmitStoreToMetadata(Builder.CreateBitCast(SLocPtr, Int8PtrTy),
                        ReturnLocation);
  }
  ...
  if (getLangOpts().ElideConstructors &&
    ...
    EmitStoreToMetadata(Result.getScalarVal(), ReturnValue);
  } else {
    switch (getEvaluationKind(RV->getType())) {
    case TEK_Scalar:
      EmitStoreToMetadata(EmitScalarExpr(RV), ReturnValue);
      break;
    case TEK_Complex:
