...
void CodeGenFunction::EmitStoreOfScalar(llvm::Value *Value, Address Addr,
  ...
  llvm::StoreInst *Store = EmitStoreToMetadata(Value, Addr, Volatile);
  if (isNontemporal) {
    llvm::MDNode *Node =
...
void CodeGenFunction::EmitStoreThroughLValue(RValue Src, LValue Dst,
  ...
  if (!Dst.isSimple()) {
    if (Dst.isVectorElt()) {
      ...
      EmitStoreToMetadata(Vec, Dst.getVectorAddress(),
                          Dst.isVolatileQualified());
      return;
    }
...
void CodeGenFunction::EmitStoreThroughBitfieldLValue(RValue Src, LValue Dst,
  ...
  EmitStoreToMetadata(SrcVal, Ptr, Dst.isVolatileQualified());
...
void CodeGenFunction::EmitStoreThroughExtVectorComponentLValue(RValue Src,
  ...
  EmitStoreToMetadata(Vec, Dst.getExtVectorAddress(),
                      Dst.isVolatileQualified());
}
...
EmitExtVectorElementExpr(const ExtVectorElementExpr *E) {
  ...
  if (E->isArrow()) {
    ...
    EmitStoreToMetadata(Vec, VecMem);
