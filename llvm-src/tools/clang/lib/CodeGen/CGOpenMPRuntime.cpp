...
static Address castToBase(CodeGenFunction &CGF, QualType BaseTy, QualType ElTy,
  ...
  while ((BaseTy->isPointerType() || BaseTy->isReferenceType()) &&
    ...
    if (TopTmp.isValid())
      CGF.EmitStoreToMetadata(Tmp.getPointer(), TopTmp);
    else
      MostTopTmp = Tmp;
    TopTmp = Tmp;
    BaseTy = BaseTy->getPointeeType();
  ...
  if (Tmp.isValid()) {
    CGF.EmitStoreToMetadata(Addr, Tmp);
    return MostTopTmp;
  }
  ...
  if (VD && ThreadPrivateWithDefinition.count(VD) == 0) {
    ...
    if (CGM.getLangOpts().CPlusPlus && PerformInit) {
      ...
      ArgVal = CtorCGF.EmitLoadOfScalar(
          CtorCGF.GetAddrOfLocalVar(&Dst), /*Volatile=*/false,
          CGM.getContext().VoidPtrTy, Dst.getLocation());
      CtorCGF.EmitStoreToMetadata(ArgVal, CtorCGF.ReturnValue);
...
void CGOpenMPRuntime::emitSingleRegion(CodeGenFunction &CGF,
  ...
  if (!CopyprivateVars.empty()) {
    ...
    CGF.EmitStoreToMetadata(CGF.Builder.getInt32(0), DidIt);
  }
  ...
  if (DidIt.isValid()) {
    // did_it = 1;
    CGF.EmitStoreToMetadata(CGF.Builder.getInt32(1), DidIt);
  }
  Action.Done(CGF);
  // call __kmpc_copyprivate(ident_t *, gtid, <buf_size>, <copyprivate list>,
  // <copy_func>, did_it);
...
void CGOpenMPRuntime::emitReduction(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  for (unsigned I = 0, E = RHSExprs.size(); I < E; ++I, ++IPriv, ++Idx) {
    ...
    if ((*IPriv)->getType()->isVariablyModifiedType()) {
      ...
      CGF.EmitStoreToMetadata(CGF.Builder.CreateIntToPtr(Size, CGF.VoidPtrTy),
                              Elem);
    }
  }
...
void CGOpenMPRuntime::emitTaskReductionFixups(CodeGenFunction &CGF,
  ...
  if (Sizes.second) {
    ...
    Address SizeAddr = getAddrOfArtificialThreadPrivate(
        CGF, CGM.getContext().getSizeType(),
        generateUniqueName(CGM, "reduction_size", RCG.getRefExpr(N)));
    CGF.EmitStoreToMetadata(SizeVal, SizeAddr, /*IsVolatile=*/false);
  }
...
emitOffloadingArrays(CodeGenFunction &CGF,
  ...
  if (Info.NumberOfPtrs) {
    ...
    for (unsigned I = 0; I < Info.NumberOfPtrs; ++I) {
      ...
      CGF.EmitStoreToMetadata(BPVal, BPAddr);
      ...
      CGF.EmitStoreToMetadata(PVal, PAddr);
