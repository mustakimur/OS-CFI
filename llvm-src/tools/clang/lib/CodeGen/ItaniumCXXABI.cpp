...
ItaniumCXXABI::buildStructorSignature(const CXXMethodDecl *MD, StructorType T,
  ...
  if (isa<CXXConstructorDecl>(MD)) {
    ...
    if (!(classDecl->isCXX11StandardLayout() || classDecl->isStandardLayout() ||
      ...
      // cerr << "======== [OSCFI] buildStructorSignature [OSCFI] ========"
      //     << endl;
      // cerr << "@ Constructor is class of:\t" << classDecl->getNameAsString()
      //     << endl;
      i++;
  ...
  if (i > 0)
    return AddedStructorArgs::prefix(i);
  else
...
void ItaniumCXXABI::addImplicitStructorParams(CodeGenFunction &CGF,
  ...
  if (isa<CXXConstructorDecl>(MD)) {
    ...
    if (!(classDecl->isCXX11StandardLayout() || classDecl->isStandardLayout() ||
          classDecl->isInStdNamespace())) {
      // cerr << "======== [OSCFI] addImplicitStructorParams [OSCFI] ========"
      //     << endl;
      // cerr << "@ Constructor is Class of:\t" << classDecl->getNameAsString()
      //     << endl;
      i++;
...
void ItaniumCXXABI::EmitInstanceFunctionProlog(
    CodeGenFunction &CGF) { // Naked functions have no prolog.
  ...
  if (HasThisReturn(CGF.CurGD)) {
    CGF.EmitStoreToMetadata(getThisValue(CGF), CGF.ReturnValue);
  }
}
...
CGCXXABI::AddedStructorArgs ItaniumCXXABI::addImplicitConstructorArgs(
  ...
  if (NeedsVTTParameter(GlobalDecl(D, Type))) {
    ...
    Args.insert(Args.begin() + i, CallArg(RValue::get(VTT), VTTTy));
  }
  ...
  if (!(classDecl->isCXX11StandardLayout() || classDecl->isStandardLayout() ||
        classDecl->isInStdNamespace()) &&
      !Delegating) {
    // cerr << "======== [OSCFI] addImplicitConstructorArgs [OSCFI] ========"
    //     << endl;
    // cerr << "@ Constructor is class of:\t" << classDecl->getNameAsString()
    //     << endl;
    ...
    if (CGF.CurFuncDecl && isa<CXXConstructorDecl>(CGF.CurFuncDecl) &&
        ...
        getStructorImpObjAllocCtxParamDecl(CGF)) {
      // cerr << "@ [OSCFI] Child calls parents constructor" << endl;
      // cerr << "@ Parent Class:\t" << classDecl->getNameAsString() << endl;
...
CGCallee ItaniumCXXABI::getVirtualFunctionPointer(CodeGenFunction &CGF,
  ...
  if (CGF.ShouldEmitVTableTypeCheckedLoad(MethodDecl->getParent())) {
    // cerr << "@ getVirtualFunctionPointer:(1)" << endl;
    VFunc = CGF.EmitVTableTypeCheckedLoad(
        MethodDecl->getParent(), VTable,
        VTableIndex * CGM.getContext().getTargetInfo().getPointerWidth(0) / 8);
  } else {
    // cerr << "@ getVirtualFunctionPointer:(2)" << endl;
...
Address ItaniumCXXABI::InitializeArrayCookie(CodeGenFunction &CGF,
  ...
  llvm::Instruction *SI = CGF.EmitStoreToMetadata(NumElements, NumElementsPtr);
...
Address ARMCXXABI::InitializeArrayCookie(CodeGenFunction &CGF,
  ...
  CGF.EmitStoreToMetadata(elementSize, cookie);

  // The second element is the element count.
  cookie = CGF.Builder.CreateConstInBoundsGEP(cookie, 1, CGF.getSizeSize());
  CGF.EmitStoreToMetadata(numElements, cookie);
  ...
  if (isa<ReferenceType>(CatchType)) {
    ...
    if (const PointerType *PT = dyn_cast<PointerType>(CaughtType)) {
      ...
      // pad.  The best solution is to fix the personality function.
        ...
        CGF.EmitStoreToMetadata(Casted, ExnPtrTmp);
    ...
    CGF.EmitStoreToMetadata(ExnCast, ParamAddr);
    return;
  }
  ...
  if (TEK != TEK_Aggregate) {
    ...
    if (CatchType->hasPointerRepresentation()) {
      ...
      case Qualifiers::OCL_Autoreleasing:
        CGF.EmitStoreToMetadata(CastExn, ParamAddr);
        return;
