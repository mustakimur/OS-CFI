...
DominatingValue<RValue>::saved_type::save(CodeGenFunction &CGF, RValue rv) {
  if (rv.isScalar()) {
    ...
    CGF.EmitStoreToMetadata(V, addr);
  ...
  CGF.EmitStoreToMetadata(V.getPointer(), addr);
...
Address CodeGenFunction::createCleanupActiveFlag() {
  ...
  EmitStoreToMetadata(Builder.getTrue(), active);
    ...
    // the cleanup block and then try to clean up after ourselves.
      ...
      if (HasFallthrough) {
        if (!HasPrebranchedFallthrough)
          EmitStoreToMetadata(Builder.getInt32(0), getNormalCleanupDestSlot());
...
static void SetupCleanupBlockActivation(CodeGenFunction &CGF,
  ...
  CGF.EmitStoreToMetadata(CGF.Builder.getInt1(kind == ForActivation), var);
}
