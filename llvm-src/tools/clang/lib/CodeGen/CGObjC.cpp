...
RValue CodeGenFunction::EmitObjCMessageExpr(const ObjCMessageExpr *E,
  ...
  if (isDelegateInit) {
    ...
    EmitStoreToMetadata(getNullForVariable(selfAddr), selfAddr);
  }
  ...
  if (isDelegateInit) {
    ...
    EmitStoreToMetadata(newSelf, selfAddr);
  }
...
CodeGenFunction::generateObjCSetterBody(const ObjCImplementationDecl *classImpl,
  ...
  case PropertyImplStrategy::Native: {
    ...
    llvm::StoreInst *store = EmitStoreToMetadata(load, ivarAddr);
...
void CodeGenFunction::EmitARCInitWeak(Address addr, llvm::Value *value) {
  ...
  if (isa<llvm::ConstantPointerNull>(value) &&
      CGM.getCodeGenOpts().OptimizationLevel == 0) {
    EmitStoreToMetadata(value, addr);
    return;
  }
